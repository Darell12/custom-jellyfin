...
[ApiController]
[Route("Api")]
public class VAController : BaseJellyfinApiController
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="VAController"/> class.
    /// </summary>
        ...
        [FromQuery] Dictionary<string, string> streamOptions)
    {
        ...
        var state = await StreamingHelpers
            .GetStreamingState(
                streamingRequest,
                HttpContext,
                _mediaSourceManager,
        ...
        {
            StreamingHelpers.AddDlnaHeaders(
                state,
                Response.Headers,
                true,
            ...
            var liveStreamInfo = _mediaSourceManager.GetLiveStreamInfo(
                streamingRequest.LiveStreamId);
        ...
        if (@static.HasValue && @static.Value && state.InputProtocol == MediaProtocol.Http)
        {
            ...
            return await FileStreamResponseHelpers
                .GetStaticRemoteStreamResult(state, httpClient, HttpContext)
                .ConfigureAwait(false);
        }
        ...
        {
            return BadRequest(
                $"Input protocol {state.InputProtocol} cannot be streamed statically");
        }

        var outputPath = state.OutputFilePath;
        var outputPathExists = System.IO.File.Exists(outputPath);

        var transcodingJob = _transcodingJobHelper.GetTranscodingJob(
            outputPath,
        ...
        StreamingHelpers.AddDlnaHeaders(
            state,
            Response.Headers,
            (@static.HasValue && @static.Value) || isTranscodeCached,
            state.Request.StartTimeTicks,
            Request,
            _dlnaManager);
        ...
        if (@static.HasValue && @static.Value)
        {
            ...
            {
                var liveStream = new ProgressiveFileStream(
                    state.MediaPath,
                    null,
                    _transcodingJobHelper);
    ...
    {
    Console.WriteLine($"Valor de elementoId: {elementoId.Value}");
    ...
    /// <summary>
    /// Metodo que retorna un hola mundo.
    /// </summary>
    /// <returns>A Retorna un mensaje simple.</returns>
    [HttpGet("hola", Name = "OldUrlEndpoint")]
    [Route("redirect")]
    public ActionResult<string> OldUrlAction()
    {
    ...
    /// <returns>A with "Hello World" message.</returns>
    [HttpGet("redirect", Name = "NewUrlEndpoint")]
    public ActionResult<string> NewUrlAction()
    {
        ...
        return Ok("¡Has llegado a la nueva URL!");
    }
    ...
    /// <response code="200">Video stream returned.</response>
    /// <returns>A <see cref="FileResult"/> containing the audio file.</returns>
    [HttpGet("/streame.{container}")]
    [HttpHead("/streame.{container}", Name = "HeadVideoStreamByContainer")]
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesVideoFile]
    public Task<ActionResult> GetVideoStreamByContainer2(
        [FromRoute, Required] string container,
        [FromQuery] bool? @static,
        [FromQuery] string? @params,
        [FromQuery] string? tag,
    ...
    {
    Guid? elementoId = HttpContext.Items["IdElemento"] as Guid?;
    Console.WriteLine("Controlador activado streame");
    if (elementoId.HasValue)
    {
    Console.WriteLine($"Valor de elementoId: {elementoId.Value}");
    return GetVideoStream(
    ...
    {
        Console.WriteLine("elementoId es nulo");
        return Task.FromResult<ActionResult>(NoContent());
    }
    }
}
